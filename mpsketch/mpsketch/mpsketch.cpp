...
void minHash(unsigned int **hashParameters, unsigned int *divisor, int k, unsigned int *features, int featureNum, unsigned int* minHashValues, unsigned int* minHashResults){
	...
	if(features[0] == MAX_FEATURE_NUM || featureNum==0) {
...
void passMessages(int **network, int *adjNum, int networkSize, int iteration, int k, 
	...
	for(int iR=1; iR<iteration+1; iR++) {
		...
		for(int iNode=0; iNode<networkSize; iNode++) {
			...
			u1Message = new unsigned int[k];
			u1MinHashResult = new unsigned int[k];
			minHash(hashU1ParametersThreeArrays[iR], divisor, k, features, featureNum, u1Message, u1MinHashResult);
			...
			u2Message = new unsigned int[k];
			u2MinHashResult = new unsigned int[k];
			minHash(hashU2ParametersThreeArrays[iR], divisor, k, sumMessages, k*adjNum[iNode], u2Message, u2MinHashResult);

			for(int iK=0; iK<k; iK++) {

				if(u1MinHashResult[iK] <= u2MinHashResult[iK]) {

					fingerprints[iNode][iK] = u1Message[iK];
				} else {

					fingerprints[iNode][iK] = u2Message[iK];
				}
			}
